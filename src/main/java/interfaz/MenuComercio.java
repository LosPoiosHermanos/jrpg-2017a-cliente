...
public class MenuComercio {
	...
	public void graficar(Graphics g) {
		...
		// listo inventarios
		// Pantalla.centerString(g, new Rectangle(280, 170 ,
		// Recursos.menuComercio.getWidth(), 0),
		...
		for (Objeto obj : personaje.getInventario().getObjetos()) {
			if (obj.getId() > 0) {
				Pantalla.centerString(g, new Rectangle(280, (200 + posicion), Recursos.menuComercio.getWidth(), 0),
						obj.getId() + " - " + obj.getNombre() + " -> " + obj.getAtributoModificado() + " + "
								+ obj.getValor());
				posicion += 20;
			}
		}
		...
		if (habilitado)
			...
			Pantalla.centerString(g, new Rectangle(x, y + 15, Recursos.menuComercioDeshabilitado.getWidth(), 0),
					"Turno Rival");
